...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function ConfettiPaper(_x, _y) {
    ...
    this.rotationSpeed = ran() * 600 + 800;
    this.angle = DEG_TO_RAD * ran() * 360;
    this.rotation = DEG_TO_RAD * ran() * 360;
    this.cosA = 1.0;
    this.size = 5.0;
    this.oscillationSpeed = ran() * 1.5 + 0.5;
    this.xSpeed = 40.0;
    this.ySpeed = ran() * 60 + 50.0;
    this.corners = new Array();
    this.time = ran();
    var ci = round(ran() * (colors.length - 1));
    ...
    this.Update = function (_dt) {
      ...
      if (this.pos.y > ConfettiPaper.bounds.y) {
        this.pos.x = ran() * ConfettiPaper.bounds.x;
        this.pos.y = 0;
      }
    };
  ...
  function ConfettiRibbon(
    ...
    this.velocityInherit = ran() * 2 + 4;
    this.time = ran() * 100;
    this.oscillationSpeed = ran() * 2 + 2;
    this.oscillationDistance = ran() * 40 + 40;
    this.ySpeed = ran() * 40 + 80;
    ...
    this.Reset = function () {
      this.position.y = -ran() * ConfettiRibbon.bounds.y;
      this.position.x = ran() * ConfettiRibbon.bounds.x;
      this.prevPosition = new Vector2(this.position.x, this.position.y);
      this.velocityInherit = ran() * 2 + 4;
      this.time = ran() * 100;
      this.oscillationSpeed = ran() * 2.0 + 1.5;
      this.oscillationDistance = ran() * 40 + 40;
      this.ySpeed = ran() * 40 + 80;
      var ci = round(ran() * (colors.length - 1));
      this.frontColor = colors[ci][0];
